...
For the sub-log containing only appeals cases, a different mining approach was used. Instead of the Inductive Miner, the Interactive Data-Aware Heuristic Miner was used. This was done because the Inductive Miner would produce a hard to comprehend complex model that allowed for a great amount of additional behaviour. Most of the activities were used in a parallel construct that completely removed any ordering from the process execution. Additionally the \texttt{Appeal to Judge} activity was optional, but could be combined with the appeal to prefecture flow. This behaviour is not wanted, as the two types of appeals are completely separate from another. The obtained model for the appeal cases can be seen in figure \ref{fig:idhm_appeals}.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/idhm_appeals.png}
  \caption{Process model discovered from the log containing only appeals using the Interactive Data-Aware Heuristic Miner.}
  \label{fig:idhm_appeals}
\end{figure}
The Interactive Data-Aware Heuristic Miner was chosen because it allows for a direct manipulation of the algorithm parameters. In this way the parameters could be tweaked in such a way, that a simple process model was obtained, that separates the two possible flows and gives a clear ordering to the activities. Again, because of its simplicity the model is not able to generalize very well. 
