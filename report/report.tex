...
\texttt{Add penalty} event. The event happens never more than once per case. The base rule for an added penalty is doubling the base amount. $\$ 1.50$ is then added in roughly one third of \texttt{Add penalty} occurrences and nothing is added in approximately $20\% $ of occurrences. Task c deals with the variations that do not fall in the biggest two categories.
